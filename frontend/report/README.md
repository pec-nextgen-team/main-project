...
**Done in this pass** — `src/services/complaintDetailsApi.js` implements
the contract below and `src/pages/ComplaintDetailsForm.jsx` now calls it
for real (loading state on mount, real success/failure gating on both
buttons, no optimistic UI). These routes are unverified against a live
backend (no backend code ships with this delivery) — confirm they exist
before shipping; a missing route now surfaces as a visible on-screen
error instead of a silently-faked "moved to Repair Assignment" message.

- `GET /api/complaints/:id` — replaces `initialComplaintForm` /
  `initialInspectionForm`, fetched on mount from the `complaintId` prop.
- `PATCH /api/complaints/:id` (partial save, stage unchanged) —
  `handleSaveDraft`.
- `POST /api/complaints/:id/advance-stage` with
  `{ from: "INSPECTION", to: "REPAIR_ASSIGNED" }`, matching the
  `ComplaintStage` model from the spec — keep the "required fields"
  validation server-side too, not just in the client.
- The right-rail cards now come from the same `GET /api/complaints/:id`
  response plus `GET /api/complaints?department=&limit=4` for Recent
  Complaints.

`ComplaintDetailsForm` takes a `complaintId` prop (not a hard
react-router-dom dependency, since this module ships as loose files).
Wire it from your route once dropped in — see the component's own
top-of-file comment for the one-line example.
