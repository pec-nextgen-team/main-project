...
- **Raise New Complaint** (`/`) — Faculty/Staff register a complaint about an
  accessory/equipment that needs repair.
- **My Complaints** (`/my-complaints`) — Faculty/Staff search, filter, paginate,
  view details of, and export their own complaints.
- **Stock In — Receive Items** (`/stock-management/stock-in`) — the Electrician
  Head records goods received against a purchase order: receipt/invoice details,
  a line-item table with a running grand total, additional receiving info, and
  invoice/delivery-note uploads, then saves a draft or confirms the stock-in.

Different pages are used by different signed-in personas (see
`src/hooks/useAuth.js` / `ROLE_BY_PATH` in `src/App.jsx`) — the header and
pre-filled fields change accordingly (e.g. "Mr. Prakash, Faculty / Staff" on the
complaint pages vs. "Mr. Selvaraj, Electrician Head" on Stock In).
...
src/
  api/
    ...
    stockApi.js               # POST /api/stock/stock-in (+ /draft)
  data/
    sampleComplaints.js       # 12 realistic accessory-repair sample records
    stockInSample.js           # dropdown options + sample Stock In receipt
  utils/
    sla.js                    # 3-day SLA math shared by the table + modal
    csvExport.js               # client-side CSV export for "Export"
    currency.js                 # ₹ (INR) number formatting
  components/
    Sidebar.jsx               # navy nav w/ collapsible groups, routed active
                               # state, Quick Links
