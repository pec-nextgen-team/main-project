...
- `/stock-management/inventory` — Stock Management: Inventory (with matching `/api/inventory*` endpoints)
...
both are running the Add Electrician and Inventory pages save for real.
`npm install` in the frontend also needs `recharts` (used for the two
donut charts on the Inventory page) — it's already in `package.json`.
...
src/
  ...
  data/
    ...
    inventoryData.js      Mock inventory items, summary stats, and chart data
  utils/
...
server/
  ...
  prisma/schema.prisma     Electrician, InventoryItem, StockMovement, Supplier models
  routes/electricians.js   GET /, GET /:id, POST /
  routes/inventory.js      GET /, GET /:id, POST /, PUT /:id, DELETE /:id,
                           POST /:id/stock-in, /:id/stock-out, /:id/adjustment
...
- Sidebar Stock Management submenu expanded (Stock Overview, Inventory,
  Stock In, Stock Out, Adjustments, Suppliers) with Inventory active-highlighted;
  Attendance/Leave/Performance promoted to top-level items per this page's nav spec
- 6 summary cards (Total Items, Total Stock Value, Low Stock, Out of Stock,
  Expiring Soon, Suppliers)
- Filter card: search, category/sub-category, location, status, stock status,
  value range, plus Search/Reset/Export — functional against the mock dataset
- Inventory table with row checkboxes, category/status badges, and
  View/Edit/Delete actions; stock value is computed live (available stock ×
  unit price)
- Add Item / Edit Item modal, delete-with-confirmation modal, and a shared
  Stock In / Stock Out / Adjustment modal that updates available stock and
  recalculates status (In Stock / Low Stock / Out of Stock) automatically
- CSV export of the currently filtered list
- Functional pagination (the reference text's "of 256 records" is
  illustrative — the shipped mock dataset has 8 real rows, and pagination
  reflects whatever the current filter returns)
- Right column: Stock Status Summary donut, Stock Value Summary, Recent
  Transactions
- Below the table: Top Low Stock Items, Top Categories donut, Quick Actions,
  and a Notes card
- Donut charts built with `recharts` (added to `package.json`)
- Matching Express + Prisma backend: `InventoryItem`, `StockMovement`, and
  `Supplier` models, plus `GET/POST /api/inventory`, `GET/PUT/DELETE
  /api/inventory/:id`, and `POST /api/inventory/:id/{stock-in,stock-out,adjustment}`
